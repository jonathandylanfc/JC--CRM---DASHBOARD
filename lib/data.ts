...
export async function getAllTasks() {
  ...
  const { data } = await supabase
    .from("tasks")
    .select("id, title, description, due_date, start_time, end_time, priority, status, created_at, completed_at, recurrence, task_category, calendar_event_id, calendar_id")
    .eq("user_id", userId)
...
export async function getBudgetCategories() {
  ...
  const { data } = await supabase
    ...
    .select("id, name, type, value, base_amount, sort_order, rollover, is_catchall, linked_account, is_goal_mode, transfer_keywords, category_aliases")
    .eq("user_id", userId)
