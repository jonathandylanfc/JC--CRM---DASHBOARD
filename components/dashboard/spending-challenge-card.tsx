...
import { Plus, Trash2, Trophy, CalendarClock, TrendingUp } from "lucide-react"
...
export function SpendingChallengeCard({ initialChallenges }: Props) {
  ...
  function openNew() {
    setForm({ card_name: "", bonus_description: "", spend_target: "", spent_amount: "0", enrolled_at: "", deadline: "" })
    setEditingId(null)
    setDialogOpen(true)
  }
  ...
  async function handleSave() {
    ...
    if (editingId) {
      const res = await updateSpentAmount(editingId, payload.spent_amount)
      if (res.error) { toast.error(res.error); setSaving(false); return }
      setChallenges((prev) => prev.map((c) => c.id === editingId ? { ...c, spent_amount: payload.spent_amount } : c))
      toast.success("Updated")
    } else {
      const res = await saveSpendingChallenge(payload)
      if (res.error) { toast.error(res.error); setSaving(false); return }
      setChallenges((prev) => [{ ...payload, id: res.id! }, ...prev])
              ...
              return (
                <div key={c.id} className="space-y-2">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className={`font-semibold ${isComplete ? "text-emerald-600 dark:text-emerald-400" : "text-foreground"}`}>
                        {currency(c.spent_amount)} <span className="font-normal text-muted-foreground">of {currency(c.spend_target)}</span>
                      </span>
                    ...
                    <div className="h-2 rounded-full bg-muted overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${isComplete ? "bg-emerald-500" : isExpired ? "bg-rose-500" : "bg-amber-500"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div className="rounded-md bg-muted/50 px-2 py-1.5 text-center">
                      <p className="text-muted-foreground flex items-center justify-center gap-1"><CalendarClock className="w-3 h-3" /> Days left</p>
                      <p className={`font-semibold ${daysLeft <= 14 && !isComplete ? "text-rose-500" : ""}`}>
                        {isExpired ? "Expired" : isComplete ? "—" : daysLeft}
                      </p>
                    </div>
                    <div className="rounded-md bg-muted/50 px-2 py-1.5 text-center">
                      <p className="text-muted-foreground flex items-center justify-center gap-1"><TrendingUp className="w-3 h-3" /> Per day</p>
            ...
            {!editingId && (
              <>
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="Chase Sapphire Preferred" value={form.card_name} onChange={(e) => setForm((f) => ({ ...f, card_name: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Bonus</Label>
                  <Input placeholder="100,000 bonus points" value={form.bonus_description} onChange={(e) => setForm((f) => ({ ...f, bonus_description: e.target.value }))} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Spend target ($)</Label>
                    <Input type="number" placeholder="5000" value={form.spend_target} onChange={(e) => setForm((f) => ({ ...f, spend_target: e.target.value }))} />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">Already spent ($)</Label>
                    <Input type="number" placeholder="0" value={form.spent_amount} onChange={(e) => setForm((f) => ({ ...f, spent_amount: e.target.value }))} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
